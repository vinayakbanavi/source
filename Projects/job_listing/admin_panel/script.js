...
$(document).ready(function () {
  ...
  $(document).on("click", ".job .filter-data", function (event) {
  ...
  function clearFormData() {
    ...
    $("#jobStatusNo").prop("checked", true);
    $("#jobTypeNo").prop("checked", true);

    $(".col2").hide();

    $(".display input[type='text']").css({
      background: "hsla(180, 10%, 47%, 0.733)",
      color: "black",
    });
  }
  ...
  function formValidation() {
    ...
    if (company.val() == "") {
      company.focus();
      company.css({ background: "rgba(0, 0, 0, 0.733)", color: "white" });
      return false;
    } else {
      company.css("background", "hsla(180, 10%, 47%, 0.733)");
      return true;
    }

    let jobTitle = $("#jobRole");
    let updated = $("#updated");
    let employment = $("#employment");
    let location = $("#location");

    let roleData = $("#roleData");
    let experienceLevel = $("#levelData");
  ...
  function validate(inputId) {
    let formInput = $("#" + inputId);

    if (formInput.val() == "") {
      formInput.focus();
      formInput.css({ background: "rgba(0, 0, 0, 0.733)", color: "white" });
      return false;
    } else {
      formInput.css({
        background: "hsla(180, 10%, 47%, 0.733)",
  ...
  $(".display input[type='text']").blur(function () {
    $(".display input[type='text']").css({
      background: "hsla(180, 10%, 47%, 0.733)",
      color: "black",
    });
